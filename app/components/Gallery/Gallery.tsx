...
export default function Gallery({ params }: { params: { reddit: string } }) {
    ...
    const fetchData = async (afterParam = '') => {
        ...
        try {
            const result = await fetchSubReddit(subReddit, isMobile ? 50 : 10, '', '') as RedditResponse;
    ...
    const fetchDataAfterBackground = async (after: string) => {
        ...
        try {
            const result = await fetchSubReddit(subReddit, isMobile ? 50 : 10, after, '') as RedditResponse;
            const data = result.data.children;
